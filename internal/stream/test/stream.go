...
func (buf *BufInfo) RunSync(path string, tmMult int, cncl context.CancelFunc) {
        var tm time.Time
        for {
...
func (buf *BufInfo) SyncFiles(path string, modTime *time.Time) error {
	...
	if err != nil {
		//slog.Warn("Can't get file names", "From path", path,
	        //          "Error", err)
		return nil
	}
...
func (buf *BufInfo) updateFromBuf(path string, arr *[]string) {
	for _, name := range *arr {
		...
		if fInf.findWhere(path) {
			...
			buf.addInTomb(name)
			//slog.Info("Delete file from bufer", "From path", path,
			//          "File", name)
		} else { 
			...
			//slog.Info("Add file in dir", "To path", path, 
			//          "File", name)
		}
		...
		if err != nil {
			...
			continue
                ...
                if fInf == nil {
                        ...
                        //slog.Info("Add in bufer", "From", path,
                        //          "File", name)
                        continue
                }
                ...
                if !fInf.IsDir && !fInf.compareInfo(&info) {
                        ...
                        if fInf.findWhere(path) {
                                ...
                                err = buf.BuildFile(path, name, fInf)
                                if err != nil {
                                        //slog.Error("Build error", "Path",
                                        //           path, "File", name,
                                        //         "Error", err)
                                        continue
                                }
                                buf.addWherePath(path, name)
                                //slog.Info("Rebuild file", "Path", path,
                                //          "File", name)
                                continue
                        }
                }
                ...
                if buf.inTomb(name) {
                        ...
                        err := os.RemoveAll(fullPath)
                        if err != nil {
                                //slog.Error("Remove error", "Path", path,
                                //           "File", name, "Error", err)
                                continue
                        }
                        //slog.Info("File deleted", "From path",
                        //          path, "File", name)
                        continue
                }
